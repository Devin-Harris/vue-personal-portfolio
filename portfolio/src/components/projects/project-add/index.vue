  ...
  <div class="project-add-container">
    ...
    <div class="project_container__add-selections">
      <simple-dropdown
        ...
        @selected-change="$emit('selected-category-change', $event)"
      />
      <simple-dropdown
        ...
        @selected-change="$emit('selected-sub-change', $event)"
      />
      ...
      >
        ...
        <div class="all-image-links" v-if="projectImages.length > 0">
          <div
            ...
            :key="imageValue"
            @click="$emit('open-image', imageValue)"
          >
            <p>{{ imageValue }}</p>
            <i
              @click.stop="$emit('remove-image', imageValue)"
              class="fas fa-trash"
            ></i>
          </div>
        </div>
        <input
          ...
          autocomplete="off"
          @keydown.enter.prevent="$emit('add-image', $event)"
        />
      </div>
      ...
      <div class="project-action-button">
        <editor-button
          ...
          :canClick="canClick"
          @editor-action-click="$emit('editor-action-click', project)"
        />
      </div>
    </div>
  </div>
