...
export default {
   ...
   methods: {
      ...
      triggerIllustrationAnimation() {
         this.resetTimeLine = !this.resetTimeLine;
      },
      cardInHover(e) {
   ...
   mounted() {
      ...
      if (this.isIllustrationShown) {
         const picElm = document.querySelector(
            '.page-heading-container__image'
         );
         this.linksTimeline
            .add({
               ...
               right: [
                  0,
                  picElm ? picElm.getBoundingClientRect().width - 16 + 'px' : 0,
               ],
               margin: 0,
               opacity: [0, 1],
               duration: 0,
